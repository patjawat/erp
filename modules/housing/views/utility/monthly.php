...
<div class="d-flex flex-wrap gap-2 expense-actions"><?=Html::a('<i class="bi bi-clock-history"></i> ประวัติรับชำระ',['/housing/payment/index'],['class'=>'btn btn-outline-secondary'])?> <?=Html::a('ตั้งค่าประเภทค่าใช้จ่าย',['charge-types'],['class'=>'btn btn-outline-secondary'])?> <?=Html::a('<i class="bi bi-calendar-plus"></i> สร้างเดือนใหม่',['create-period'],['class'=>'btn btn-outline-primary open-modal','data-size'=>'modal-lg'])?></div>
</div>
...
<div class="d-flex gap-2"><?php if(!$accounts&&$period->status==='open'):?><?=Html::a('<i class="bi bi-list-check"></i> ดึงข้อมูลบ้านพักทั้งหมด',['generate-month','period_id'=>$period->id],['class'=>'btn btn-primary','data-method'=>'post','data-confirm'=>'สร้างรายการจากข้อมูลบ้านพักและผู้พักอาศัยปัจจุบันหรือไม่?'])?><?php endif;?>
<?php if($accounts):?><?=Html::a('<i class="bi bi-bell"></i> แจ้งค่าใช้จ่ายให้ผู้พัก',['notify-expenses','period_id'=>$period->id],['class'=>'btn btn-outline-primary','data-method'=>'post','data-confirm'=>'ส่งแจ้งเตือนค่าใช้จ่ายและกำหนดชำระ ให้ผู้พักทุกรายที่บันทึกยอดแล้ว (แจ้งในแอป + Telegram) ยืนยันหรือไม่?'])?><?php endif;?>
<?php if($accounts&&$period->status==='open'):?><?=Html::a('<i class="bi bi-people"></i> อัปเดตจำนวนผู้พัก',['refresh-occupants','period_id'=>$period->id],['class'=>'btn btn-outline-secondary','data-method'=>'post','data-confirm'=>'ดึงจำนวนผู้พักอาศัยล่าสุดจากทะเบียนผู้พักมาปรับในรอบนี้หรือไม่?'])?> <?=Html::a('<i class="bi bi-lock"></i> ปิดค่าใช้จ่ายประจำเดือน',['close-period','id'=>$period->id],['class'=>'btn btn-outline-danger','data-method'=>'post','data-confirm'=>'เมื่อปิดรอบแล้วจะไม่สามารถแก้ไขข้อมูลได้ ยืนยันหรือไม่?'])?><?php endif;?></div>
</div>
...
<div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>บ้านพัก/ห้อง</th><th>หมายเลขผู้ใช้ไฟฟ้า</th><th>เจ้าหน้าที่</th><th class="text-center">ผู้พักอาศัย</th><th class="text-end">ค่าใช้จ่าย</th><th class="text-end">ชำระแล้ว</th><th class="text-end">คงเหลือ</th><th>สถานะ</th><th></th></tr></thead><tbody>
...
<td class="text-center"><?php if($account->occupancy_id):?><span class="fw-semibold"><?=(int)$account->occupants_total?> คน</span><div class="small text-body-secondary">เกิน 15 ปี <?=(int)$account->occupants_over_15?></div><?php else:?><span class="text-body-secondary">—</span><?php endif;?></td><td class="text-end"><?=Yii::$app->formatter->asDecimal($account->total_amount,2)?></td><td class="text-end"><?=Yii::$app->formatter->asDecimal($account->paid_amount,2)?></td><td class="text-end fw-semibold"><?=Yii::$app->formatter->asDecimal($account->balance_amount,2)?></td>
...
<?php if($period->status==='closed'&&$account->status===MonthlyAccount::STATUS_SAVED&&$account->occupancy_id&&(float)$account->balance_amount>0):?><?=Html::a('<i class="bi bi-cash-coin"></i> รับชำระ',['/housing/payment/receive','account_id'=>$account->id],['class'=>'btn btn-sm btn-success open-modal','data-size'=>'modal-lg'])?><?php endif;?>
...
<?php if($period->status==='closed'):?><div class="p-3 border-top small text-body-secondary"><i class="bi bi-lock"></i> ปิดรอบวันที่ <?=Yii::$app->formatter->asDatetime($period->closed_at,'php:d/m/Y H:i')?> โดย <?=Html::encode($period->closed_by_name?:('รหัสผู้ใช้ '.$period->closed_by))?> ข้อมูลถูกล็อกแล้ว</div><?php endif;?>
<?php endif;?>
</div></div>
