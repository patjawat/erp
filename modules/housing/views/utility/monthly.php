...
?>
<style>
.expense-month{--e-bg:#f7fafc;--e-soft:#eef5ff;--e-border:#dce6f0;--e-ink:#26384a;--e-muted:#60758a;--e-primary:#4c84c6;color:var(--e-ink)}
.expense-month .panel{background:#fff;border:1px solid var(--e-border);border-radius:.85rem}
...
.status-pending,.payment-unpaid{background:#fff4dc;color:#805d14}.status-saved,.payment-paid{background:#e8f5ee;color:#287a51}.payment-partial{background:#eef5ff;color:#356b9d}
...
<div><h1 class="h5 mb-1">รายการบ้านพักและผู้รับผิดชอบค่าใช้จ่าย</h1><div class="small text-muted">สร้างข้อมูลของเดือนก่อน แล้วกดลงค่าใช้จ่ายทีละคนหรือห้องว่าง</div></div>
<div class="d-flex flex-wrap gap-2 expense-actions"><?=Html::a('ตั้งค่าประเภทค่าใช้จ่าย',['charge-types'],['class'=>'btn btn-outline-secondary'])?> <?=Html::a('<i data-lucide="calendar-plus"></i> สร้างเดือนใหม่',['create-period'],['class'=>'btn btn-outline-primary open-modal','data-size'=>'modal-lg'])?></div>
</div>
...
<?php if(!$period):?><div class="text-center py-5"><div class="fw-semibold">ยังไม่มีรอบค่าใช้จ่าย</div><div class="small text-muted mt-1">กด “สร้างเดือนใหม่” เพื่อเริ่มต้น</div></div>
<?php else:?>
<div class="px-3 py-2 d-flex flex-wrap justify-content-between align-items-center gap-2">
<div><strong><?=Html::encode($period->name)?></strong><span class="text-muted ms-2"><?=$period->status==='closed'?'ปิดรอบแล้ว':'กำลังจัดทำ'?></span></div>
...
<?php if($accounts):?><div class="summary"><div><span class="small text-muted">ค่าใช้จ่ายรวม</span><strong><?=Yii::$app->formatter->asDecimal($summary['total'],2)?> บาท</strong></div><div><span class="small text-muted">ชำระแล้ว</span><strong><?=Yii::$app->formatter->asDecimal($summary['paid'],2)?> บาท</strong></div><div><span class="small text-muted">คงเหลือ</span><strong><?=Yii::$app->formatter->asDecimal($summary['balance'],2)?> บาท</strong></div><div><span class="small text-muted">ยังไม่ลงรายการ</span><strong><?=$summary['pending']?> รายการ</strong></div><div><span class="small text-muted">ยังชำระไม่ครบ</span><strong><?=$summary['unpaid']?> รายการ</strong></div></div>
...
<td><strong><?=Html::encode($account->building_name)?></strong><div class="small text-muted"><?=Html::encode(implode(' / ',array_filter([$account->unit_name,$account->room_name]))?:'ทั้งหลัง')?></div></td>
<td><?=Html::encode($account->electric_account_no?:'ยังไม่ระบุ')?></td>
<td><strong><?=Html::encode($account->payer_name?:'ห้องว่าง')?></strong><div class="small text-muted"><?=Html::encode($account->position_name?:($account->payer_name?'ไม่ระบุตำแหน่ง':'บันทึกค่าใช้จ่ายประจำห้อง'))?></div></td>
...
<td class="text-end"><?php if($period->status==='open'):?><?=Html::a($account->status===MonthlyAccount::STATUS_SAVED?'แก้ไข':'ลงค่าใช้จ่าย',['edit-account','id'=>$account->id],['class'=>'btn btn-sm btn-outline-primary open-modal','data-size'=>'modal-xl'])?><?php else:?><?=Html::a('ดูรายละเอียด',['edit-account','id'=>$account->id],['class'=>'btn btn-sm btn-outline-secondary open-modal','data-size'=>'modal-xl'])?><?php endif;?></td>
</tr><?php endforeach;?></tbody></table></div>
<?php else:?><div class="text-center py-5"><div class="fw-semibold">พร้อมสร้างรายการของเดือนนี้</div><div class="small text-muted mt-1">ระบบจะดึงบ้านพัก ห้อง เจ้าหน้าที่ และห้องว่างทั้งหมดในครั้งเดียว</div></div><?php endif;?>
<?php if($period->status==='closed'):?><div class="p-3 border-top small text-muted"><i data-lucide="lock"></i> ปิดรอบวันที่ <?=Yii::$app->formatter->asDatetime($period->closed_at,'php:d/m/Y H:i')?> โดย <?=Html::encode($period->closed_by_name?:('รหัสผู้ใช้ '.$period->closed_by))?> ข้อมูลถูกล็อกแล้ว</div><?php endif;?>
<?php endif;?>
</div></div>
