...
<?php if(!$models):?><tr><td colspan="8" class="text-center py-5"><strong>ยังไม่มีประวัติรับชำระ</strong><div class="small text-body-secondary mt-1">รายการจะปรากฏหลังรับชำระจากหน้าค่าใช้จ่ายประจำเดือน</div></td></tr><?php endif;?>
</tbody></table></div>
<ul class="list-group list-group-flush d-lg-none"><?php foreach($models as $model):?><li class="list-group-item p-3"><div class="d-flex justify-content-between"><strong><?= Html::encode($model->receipt?->receipt_no) ?></strong><span><?= Yii::$app->formatter->asDecimal($model->amount,2) ?> บาท</span></div><div class="small text-body-secondary mt-1"><?= Yii::$app->formatter->asDatetime($model->paid_at,'php:d/m/Y H:i') ?></div><?= Html::a('รายละเอียด',['view','id'=>$model->id],['class'=>'btn btn-sm btn-outline-primary mt-2']) ?></li><?php endforeach;?></ul>
