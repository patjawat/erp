...
<div><div class="fw-semibold">รายการคำขอ</div><div class="small text-muted">ตรวจสอบ บันทึกผลมติ และจัดสรรที่พัก</div></div>
...
<td><strong><?= Html::encode($profile['name'] ?? 'บุคลากร #' . $model->emp_id) ?></strong><div class="small text-muted"><?= Html::encode($profile['gender'] ?? 'ไม่ระบุ') ?></div></td>
<td><div><?= Html::encode($profile['position'] ?? 'ไม่ระบุ') ?></div><div class="small text-muted"><?= Html::encode($profile['department'] ?? 'ไม่ระบุ') ?></div></td>
...
<ul class="list-group list-group-flush d-lg-none"><?php foreach ($dataProvider->models as $model): $profile = $employeeProfiles[(int)$model->emp_id] ?? null; ?><li class="list-group-item py-3"><?= Html::a(Html::encode($model->request_no), ['view', 'id' => $model->id], ['class' => 'fw-semibold text-decoration-none']) ?><div class="mt-1 fw-semibold"><?= Html::encode($profile['name'] ?? 'บุคลากร #' . $model->emp_id) ?></div><div class="small text-muted"><?= Html::encode(implode(' · ', array_filter([$profile['gender'] ?? null, $profile['position'] ?? null, $profile['department'] ?? null]))) ?></div><div class="small mt-1"><?= Html::encode(HousingRequest::statusOptions()[$model->status] ?? '') ?></div></li><?php endforeach; ?></ul>
