...
?>
<style>
.maintenance-page{--m-bg:var(--bs-tertiary-bg);--m-border:var(--bs-border-color);--m-ink:var(--bs-emphasis-color);--m-muted:var(--bs-secondary-color);--m-primary:var(--bs-primary);color:var(--m-ink)}
.maintenance-page .soft-panel{background:var(--bs-body-bg);border:1px solid var(--m-border);border-radius:.8rem}
...
.status-new{color:var(--bs-primary);background:var(--bs-primary-bg-subtle)}.status-in_progress{color:var(--bs-warning-text-emphasis);background:var(--bs-warning-bg-subtle)}.status-completed{color:var(--bs-success-text-emphasis);background:var(--bs-success-bg-subtle)}.status-unable,.status-cancelled{color:var(--bs-danger-text-emphasis);background:var(--bs-danger-bg-subtle)}
.priority-normal{color:var(--bs-secondary-color);background:var(--bs-tertiary-bg)}.priority-urgent{color:var(--bs-warning-text-emphasis);background:var(--bs-warning-bg-subtle)}.priority-emergency{color:var(--bs-danger-text-emphasis);background:var(--bs-danger-bg-subtle)}
    ...
    <div class="soft-panel overflow-hidden">
        ...
        <?php if ($dataProvider->models): ?><div class="table-responsive"><table class="table table-hover align-middle mb-0">
            ...
            <tbody><?php foreach ($dataProvider->models as $model): ?><tr>
                ...
                <td><?= Html::encode($model->building->name ?? '—') ?><div class="small text-body-secondary"><?= Html::encode($model->location_note ?: 'ไม่ระบุจุด') ?></div></td>
                <td><strong><?= Html::encode($model->title) ?></strong><div class="small text-body-secondary"><?= Html::encode(MaintenanceRequest::reporterTypeOptions()[$model->reporter_type] ?? '') ?> · <?= Html::encode(MaintenanceRequest::scopeOptions()[$model->problem_scope] ?? '') ?></div><div class="small text-body-secondary text-truncate" style="max-width:280px"><?= Html::encode($model->description) ?></div></td>
