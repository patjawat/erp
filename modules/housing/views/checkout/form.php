...
<div class="row g-3"><div class="col-md-4"><small class="text-muted d-block">ผู้ส่งคืน</small><strong><?= Html::encode($model->resident_name) ?></strong></div><div class="col-md-8"><small class="text-muted d-block">บ้านพัก/ห้องพัก</small><strong><?= Html::encode($location) ?></strong></div><div class="col-12"><small class="text-muted d-block">เหตุผล</small><?= nl2br(Html::encode($model->move_out_reason)) ?></div></div>
</div></section>
...
<?php if ($assetItems === []): ?><div class="checkout-body text-muted">สถานที่นี้ไม่มีรายการอุปกรณ์ที่ต้องตรวจ</div><?php else: ?><div class="table-responsive"><table class="table checkout-table align-middle mb-0"><thead><tr><th>รายการ</th><th>จำนวน</th><th>สภาพเมื่อคืน</th><th>หมายเหตุ</th><th>ตรวจแล้ว</th></tr></thead><tbody>
...
<div class="d-flex justify-content-end gap-2"><?= Html::a('ยกเลิก', ['view','id'=>$model->id], ['class'=>'btn btn-light']) ?><?= Html::submitButton('บันทึกผลตรวจและส่งให้ผู้พักลงนาม', ['class'=>'btn btn-primary']) ?></div>
