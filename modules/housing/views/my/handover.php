...
?>
<style>
.my-handover{max-width:980px;margin:0 auto;--line:rgba(15,23,42,.1);--muted:#4a5568}.my-handover .surface{background:#fff;border:1px solid var(--line);border-radius:10px;box-shadow:0 1px 2px rgba(15,23,42,.04);overflow:hidden}.my-handover .surface-head{padding:.9rem 1.1rem;background:#f7f9fc;border-bottom:1px solid var(--line);font-weight:600}.my-handover .surface-body{padding:1.1rem}.my-handover .signature-state{display:flex;align-items:flex-start;gap:.65rem;padding:.8rem;border:1px solid var(--line);border-radius:8px}.my-handover .signature-state.is-done{background:rgba(21,128,61,.08);color:#166534}.my-handover .signature-state.is-wait{background:rgba(180,83,9,.08);color:#92400e}
</style>
<div class="container-fluid py-3"><div class="my-handover">
<?= Html::a('<i data-lucide="arrow-left"></i> กลับไปบ้านพักของฉัน', ['/profile', 'name' => 'housing'], ['class' => 'btn btn-outline-secondary mb-3']) ?>
...
<section class="surface"><div class="surface-head">การลงนาม</div><div class="surface-body">
    <div class="signature-state <?= $model->handed_over_signed_at ? 'is-done' : 'is-wait' ?> mb-2"><i data-lucide="<?= $model->handed_over_signed_at ? 'circle-check' : 'clock-3' ?>"></i><div><strong>ผู้ส่งมอบ: <?= Html::encode($model->handed_over_by_name) ?></strong><div class="small"><?= $model->handed_over_signed_at ? 'ลงนามแล้ว ' . Yii::$app->formatter->asDatetime($model->handed_over_signed_at, 'php:d/m/Y H:i') : 'ยังไม่ได้ลงนามส่งมอบ' ?></div></div></div>
    <div class="signature-state <?= $model->received_signed_at ? 'is-done' : 'is-wait' ?> mb-3"><i data-lucide="<?= $model->received_signed_at ? 'circle-check' : 'pen-line' ?>"></i><div><strong>ผู้รับมอบ: <?= Html::encode($model->received_by_name) ?></strong><div class="small"><?= $model->received_signed_at ? 'ลงนามแล้ว ' . Yii::$app->formatter->asDatetime($model->received_signed_at, 'php:d/m/Y H:i') : 'รอคุณตรวจและลงนาม' ?></div></div></div>
