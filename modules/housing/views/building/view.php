...
?>
<style>
.building-detail{--h-bg:var(--bs-tertiary-bg);--h-soft:var(--bs-primary-bg-subtle);--h-border:var(--bs-border-color);--h-ink:var(--bs-emphasis-color);--h-muted:var(--bs-secondary-color);--h-primary:var(--bs-primary);color:var(--h-ink)}
.building-detail .detail-hero,.building-detail .detail-section{background:var(--bs-body-bg);border:1px solid var(--h-border);border-radius:.85rem}
...
.building-detail .status-pill{display:inline-flex;padding:.25rem .6rem;border-radius:999px;font-size:.8rem;font-weight:600;background:var(--bs-primary-bg-subtle);color:var(--bs-primary)}
        ...
        <div class="p-3 p-lg-4 d-flex flex-column flex-md-row gap-4">
            ...
            <div class="flex-grow-1">
                <div class="d-flex flex-wrap justify-content-between gap-3">
                    <div>
                        <div class="d-flex flex-wrap align-items-center gap-2 mb-2"><span class="status-pill"><?= Html::encode(Building::typeOptions()[$model->building_type] ?? '') ?></span><span class="text-body-secondary"><?= Html::encode($model->code) ?></span></div>
    ...
    <div class="row g-3">
        <div class="col-xl-7">
            ...
            <section class="detail-section overflow-hidden mt-3">
                ...
                <?php else: foreach ($expenseHistory as $expense): ?><div class="unit-row p-3 d-flex justify-content-between gap-3"><div><strong><?= Html::encode($expense->period->name) ?></strong><div class="small text-body-secondary"><?= Html::encode($expense->payer_name ?: 'ห้องว่าง') ?> · <?= Html::encode($expense->unit_name ?: 'ทั้งหลัง') ?></div></div><div class="text-end"><strong><?= Yii::$app->formatter->asDecimal($expense->total_amount, 2) ?> บาท</strong><div class="small text-body-secondary">คงเหลือ <?= Yii::$app->formatter->asDecimal($expense->balance_amount, 2) ?></div></div></div><?php endforeach; endif; ?>
            </section>
        </div>
        <div class="col-xl-5">
            <section class="detail-section overflow-hidden mb-3">
                ...
                <?php else: foreach ($occupancies as $occupancy): ?>
                    <div class="unit-row p-3">
                        ...
                        <div class="small mt-1"><span class="text-body-secondary">ตำแหน่ง:</span> <?= Html::encode($occupancy->employee?->positionName() ?: 'ไม่ระบุ') ?></div>
                        <div class="small"><span class="text-body-secondary">หน่วยงาน/สถานที่ทำงาน:</span> <?= Html::encode($occupancy->employee?->departmentName() ?: 'ไม่ระบุ') ?></div>
                        <div class="small text-body-secondary"><?= Html::encode($occupancy->unit?->name ?: '') ?><?= $occupancy->room ? ' · ' . Html::encode($occupancy->room->name) : '' ?></div>
            ...
            <section class="detail-section overflow-hidden">
                ...
                <?php else: foreach ($maintenanceRequests as $request): ?>
                    <div class="unit-row p-3">
                        <div class="d-flex justify-content-between gap-2"><strong><?= Html::a(Html::encode($request->title), ['/housing/maintenance/view', 'id' => $request->id], ['class' => 'text-decoration-none']) ?></strong><span class="small text-body-secondary"><?= Yii::$app->formatter->asDate($request->reported_at, 'php:d/m/Y') ?></span></div>
                        <div class="small text-body-secondary mt-1"><?= Html::encode(MaintenanceRequest::reporterTypeOptions()[$request->reporter_type] ?? 'ผู้ดูแลบ้านพักแจ้ง') ?> · <?= Html::encode(MaintenanceRequest::scopeOptions()[$request->problem_scope] ?? 'โครงสร้าง/ภายนอกอาคาร') ?></div>
                    </div>
                <?php endforeach; endif; ?>
            </section>
        </div>
