...
{
    public function behaviors(){return array_merge(parent::behaviors(),['access'=>['class'=>AccessControl::class,'rules'=>[['allow'=>true,'actions'=>['index','view','pending'],'roles'=>['accountingView']],['allow'=>true,'actions'=>['create'],'roles'=>['accountingPrepare']],['allow'=>true,'actions'=>['post'],'roles'=>['accountingPost']],['allow'=>true,'actions'=>['settings'],'roles'=>['accountingChartManage']]]],'verbs'=>['class'=>VerbFilter::class,'actions'=>['create'=>['POST'],'post'=>['POST'],'settings'=>['GET','POST']]]]);}
    ...
    public function actionCreate($payable_id){$payable=FinancePayable::findOne($payable_id);if(!$payable)throw new NotFoundHttpException('ไม่พบทะเบียนเจ้าหนี้');try{$journal=(new AccountingJournalDraftService())->createFromPayable($payable);Yii::$app->session->setFlash('success','สร้างรายการบัญชีร่างและตรวจสอบยอดสมดุลแล้ว');return $this->redirect(['view','id'=>$journal->id]);}catch(\DomainException $e){Yii::$app->session->setFlash('warning',$e->getMessage());}catch(\Throwable $e){Yii::error($e,__METHOD__);Yii::$app->session->setFlash('error','สร้างรายการบัญชีร่างไม่สำเร็จ กรุณาติดต่อผู้ดูแลระบบ');}return $this->redirect(['pending']);}

    /** คิวเจ้าหนี้รอลงบัญชี — การเงินส่งมาแล้ว (approved + sent) และยังไม่มีสมุดรายวัน */
    public function actionPending()
    {
        $journaled = AccountingJournalDraft::find()->select('source_id')
            ->where(['source_type' => AccountingJournalDraft::SOURCE_PAYABLE]);
        $query = FinancePayable::find()
            ->where(['status' => FinancePayable::STATUS_APPROVED])
            ->andWhere(['not', ['sent_accounting_at' => null]])
            ->andWhere(['not in', 'id', $journaled])
            ->orderBy(['sent_accounting_at' => SORT_ASC, 'id' => SORT_ASC]);
        return $this->render('pending', ['dataProvider' => new ActiveDataProvider([
            'query' => $query,
            'pagination' => ['pageSize' => 30],
        ])]);
    }
    public function actionPost($id){$model=AccountingJournalDraft::findOne($id);if(!$model)throw new NotFoundHttpException('ไม่พบรายการบัญชีร่าง');try{(new AccountingPostingService())->post($model);Yii::$app->session->setFlash('success','ผ่านรายการเข้าสู่บัญชีแยกประเภทแล้ว');}catch(\DomainException $e){Yii::$app->session->setFlash('warning',$e->getMessage());}catch(\Throwable $e){Yii::error($e,__METHOD__);Yii::$app->session->setFlash('error','ผ่านรายการไม่สำเร็จ กรุณาติดต่อผู้ดูแลระบบ');}return $this->redirect(['view','id'=>$id]);}
