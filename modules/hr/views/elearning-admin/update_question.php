...
/** @var app\modules\hr\models\ElearningAnswer[] $answers */

$this->title = 'แก้ไขโจทย์ข้อสอบ: ' . $course->title;
...
$this->params['breadcrumbs'][] = 'แก้ไขโจทย์ข้อสอบ';
?>

<div class="elearning-question-update">
    <?php $this->beginBlock('page-title'); ?>
    แก้ไขโจทย์ข้อสอบหลังเรียน
    <?php $this->endBlock(); ?>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body p-4">
            ...
            <div class="row">
                <div class="col-md-9">
                    <?= $form->field($model, 'question_text')->textarea(['rows' => 3])->label('โจทย์คำถาม <span class="text-danger">*</span>') ?>
                </div>
                <div class="col-md-3">
                    <?= $form->field($model, 'sort_order')->textInput(['type' => 'number'])->label('ลำดับการแสดงผลข้อนี้') ?>
                </div>
            </div>
            ...
            <div class="answers-container">
                <?php for ($i = 0; $i < 4; $i++): ?>
                    ...
                    $ans = isset($answers[$i]) ? $answers[$i] : null;
                    $ansText = $ans ? $ans->answer_text : '';
                    $isCorrect = $ans ? $ans->is_correct : 0;
                    ?>
                    <div class="row align-items-center mb-3">
                        <div class="col-auto text-end" style="width: 80px;">
                            <div class="form-check d-flex justify-content-end align-items-center">
                                <input class="form-check-input fs-5 cursor-pointer border-warning" type="radio" name="correct_answer" id="correct_<?= $i ?>" value="<?= $i ?>" <?= $isCorrect ? 'checked' : '' ?>>
                                <label class="form-check-label fw-bold text-primary ms-2 cursor-pointer" for="correct_<?= $i ?>">เฉลย <?= $char ?></label>
                            </div>
                        </div>
                        <div class="col">
                            <?= Html::textInput("Answers[{$i}]", $ansText, [
                                'class' => 'form-control',
                                'placeholder' => "ป้อนข้อความตัวเลือกที่ {$char}",
                                'required' => true,
                            ]) ?>
                        </div>
                    </div>
