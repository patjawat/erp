...
//ระดับตำแหน่งทางราชการ จบ
    ...
    {
        return \app\modules\hr\services\WorkforceSnapshotService::sql();
    }
    ...
    {
        return \app\modules\hr\services\WorkforceSnapshotService::inServiceWhereSql($alias);
    }
    ...
    public function actionDashboard()
    {
        ...
        $engagementDashboard = ['status' => 'not_authorized', 'rounds' => [], 'report' => null];
        if (Yii::$app->user->can('engagementViewAnalytics') || Yii::$app->user->can('admin')) {
            $engagementDashboard = (new \app\modules\hr\services\EngagementService())->dashboard(
                (int) $budgetYear,
                isset($req['engagement_round']) && is_scalar($req['engagement_round']) ? (int) $req['engagement_round'] : null
            );
        }
