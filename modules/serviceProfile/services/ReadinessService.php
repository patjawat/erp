...
class ReadinessService
{
    public function inspect(ServiceProfile $profile): array
    {
        ...
        $directorReady = $this->hasEmployeeWithPermission('serviceProfileDirectorApprove');
        ...
        $checks = [
            ...
            ['key'=>'director','label'=>'ผู้อนุมัติ','ready'=>$directorReady,'detail'=>$directorReady?'พร้อมรับเอกสาร':'ยังไม่มีผู้ได้รับสิทธิอนุมัติ'],
    ...
    private function hasEmployeeWithPermission(string $permission): bool
    {
        foreach(Employees::find()->select(['id','user_id'])->andWhere(['not',['user_id'=>null]])->each(100) as $employee) {
            if(Yii::$app->authManager->checkAccess((int)$employee->user_id,$permission)) return true;
        }
        return false;
    }
}
