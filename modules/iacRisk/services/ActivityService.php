...
class ActivityService
{
    public function log(array $attributes): void
    {
        $row = new Activity($attributes + [
            'ip_address' => Yii::$app->has('request') && Yii::$app->request instanceof \yii\web\Request ? Yii::$app->request->userIP : null,
            'created_at' => date('Y-m-d H:i:s'),
            'created_by' => !Yii::$app->has('user') || Yii::$app->user->isGuest ? null : (int) Yii::$app->user->id,
        ]);
