...
// หรือใช้วิธีที่ 3 - ปรับ canvas ให้ตรงกับ A4 ตั้งแต่ต้น
// แก้ไขในส่วนโหลด PDF
pdfjsLib.getDocument('$pdfFileUrl').promise.then(pdf => {
    return pdf.getPage(1);
}).then(page => {
    // กำหนดขนาด A4 ใน pixels (สำหรับ 96 DPI)
    const a4Width = 595.28 * 96 / 72; // แปลง points เป็น pixels
    const a4Height = 841.89 * 96 / 72;
    
    // หา scale ที่เหมาะสม
    const viewport = page.getViewport({ scale: 1 });
    const scaleX = a4Width / viewport.width;
    const scaleY = a4Height / viewport.height;
    const finalScale = Math.min(scaleX, scaleY); // ใช้ scale ที่เล็กกว่าเพื่อให้พอดี
    
    const scaledViewport = page.getViewport({ scale: finalScale });
    
    pdfCanvas.height = scaledViewport.height;
    pdfCanvas.width = scaledViewport.width;
    fabricCanvasEl.height = scaledViewport.height;
    fabricCanvasEl.width = scaledViewport.width;
...
// หรือใช้วิธีนี้ถ้าต้องการให้ขนาดเท่ากับต้นฉบับ
$('#downloadPDF').on('click', function () {
    ...
    // ใช้ขนาดเดิมของ PDF (หารด้วย scale เพื่อให้ได้ขนาดจริง)
    const originalWidth = pdfCanvas.width / scale;  // scale = 1.5
    const originalHeight = pdfCanvas.height / scale;
    
    // สร้าง PDF ด้วยขนาดเดิม
    const pdf = new jsPDF('p', 'pt', [originalWidth, originalHeight]);
    
    // เพิ่มรูปภาพเต็มขนาด
    pdf.addImage(finalImage, 'PNG', 0, 0, originalWidth, originalHeight);
    pdf.save('filled-form.pdf');
});
