<?php

use app\modules\inventoryV2\models\StockDetail;
use yii\helpers\Html;
use yii\helpers\Url;
use yii\web\View;

$this->title = 'ดำเนินการจ่ายพัสดุ (Issue Process) - ' . $model->order_no;

// ดึงรายการ Lot ทั้งหมดที่เกี่ยวข้องกับพัสดุในใบเบิกนี้มาเตรียมไว้ (เพื่อลดการ Query ใน Loop)
// หรือจะ Query สดใน Loop ก็ได้เพื่อความแม่นยำของยอดคงเหลือล่าสุด
?>

<div class="container-fluid py-4">
    <div class="card shadow-sm border-0">
        <div class="card-header bg-primary text-white py-3 d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
                <i class="bi bi-box-seam fs-4"></i>
                <h5 class="mb-0 text-white">บันทึกการจ่ายพัสดุ: <?= Html::encode($model->order_no) ?></h5>
            </div>
            <span class="badge text-bg-light text-primary rounded-pill px-3">
                คลังต้นทาง: <?= Html::encode($model->mainWarehouse->warehouse_name ?? 'คลังหลัก') ?>
            </span>
        </div>

        <div class="card-body">
            <div class="row mb-4 bg-light p-3 rounded mx-0 border">
                <div class="col-md-4">
                    <small class="text-muted d-block text-uppercase fw-bold" style="font-size: 0.7rem;">หน่วยงานที่เบิก</small>
                    <strong class="text-primary"><?= Html::encode($model->subWarehouse->warehouse_name ?? '-') ?></strong>
                </div>
                <div class="col-md-4 text-center border-start border-end">
                    <small class="text-muted d-block text-uppercase fw-bold" style="font-size: 0.7rem;">อ้างอิง/หมายเหตุ</small>
                    <strong><?= Html::encode($model->source_type ?? '-') ?></strong>
                </div>
                <div class="col-md-4 text-end">
                    <small class="text-muted d-block text-uppercase fw-bold" style="font-size: 0.7rem;">วันที่เบิก</small>
                    <strong><?= Yii::$app->formatter->asDate($model->order_date, 'medium') ?></strong>
                </div>
            </div>

            <form id="issue-process-form">
                <input type="hidden" name="<?= Yii::$app->request->csrfParam ?>" value="<?= Yii::$app->request->getCsrfToken() ?>">

                <div class="table-responsive">
                    <table class="table table-hover align-middle border" id="issueTable">
                        <thead class="table-dark">
                            <tr class="text-center">
                                <th width="5%">#</th>
                                <th width="25%" class="text-start">รายการพัสดุ</th>
                                <th width="10%">ขอเบิก</th>
                                <th width="12%">จ่ายจริง</th>
                                <th width="25%">เลือกตัดจาก Lot (คลังหลัก)</th>
                                <th width="12%">รวมมูลค่า</th>
                                <th width="11%">จัดการ</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($model->stockDetails as $index => $detail): ?>
                                <?php
                                // ดึงยอดคงเหลือแยกตาม Lot ในคลังหลัก
                                $availableLots = \app\modules\inventoryV2\models\StockBalance::find()
                                    ->where(['item_code' => $detail->item_code, 'warehouse_id' => $model->main_warehouse_id])
                                    ->andWhere(['>', 'balance_qty', 0])
                                    ->all();
                                ?>
                                <tr class="item-row" data-index="<?= $index ?>">
                                    <td class="text-center text-muted"><?= $index + 1 ?></td>
                                    <td>
                                        <strong><?= Html::encode($detail->item->item_name) ?></strong><br>
                                        <small class="text-muted">Code: <?= Html::encode($detail->item_code) ?></small>
                                        <input type="hidden" name="Issue[<?= $index ?>][detail_id]" value="<?= $detail->id ?>">
                                    </td>
                                    <td class="text-center fw-bold fs-5 text-secondary"><?= number_format($detail->qty, 2) ?></td>
                                    <td>
                                        <input type="number" name="Issue[<?= $index ?>][qty_issued]"
                                            class="form-control text-center fw-bold border-primary qty-issued"
                                            value="<?= $detail->qty ?>"
                                            min="0"
                                            max="<?= $detail->qty ?>"
                                            step="0.01">
                                        <div class="invalid-feedback mt-1" style="font-size: 0.7rem;">ยอดเกินที่ขอ/ในสต็อก</div>
                                    </td>
                                    <td>
                                        <?php if (empty($availableLots)): ?>
                                            <div class="text-danger small"><i class="bi bi-exclamation-triangle"></i> ของหมดสต็อก</div>
                                            <input type="hidden" name="Issue[<?= $index ?>][lot_number]" value="">
                                        <?php else: ?>
                                            <select name="Issue[<?= $index ?>][lot_number]" class="form-select border-warning lot-selector">
                                                <?php
                                                // ดึงรายการรับเข้า (IN) ของพัสดุตัวนี้ที่มีของเหลือ เพื่อเอาราคาที่รับมาจริงใน Lot นั้น
                                                $lotsWithPrice = StockDetail::find()
                                                    ->joinWith('stockOrder')
                                                    ->where([
                                                        'stock_detail.item_code' => $detail->item_code,
                                                        'stock_order.main_warehouse_id' => $model->main_warehouse_id,
                                                        'stock_order.order_type' => 'IN'
                                                    ])
                                                    ->andWhere(['>', 'remain_qty', 0])
                                                    ->all();

                                                foreach ($lotsWithPrice as $lotIn): ?>
                                                    <option value="<?= Html::encode($lotIn->lot_number) ?>"
                                                        data-stock="<?= $lotIn->remain_qty ?>"
                                                        data-price="<?= $lotIn->unit_price ?>"> LOT: <?= Html::encode($lotIn->lot_number) ?>
                                                        (เหลือ <?= number_format($lotIn->remain_qty, 2) ?>)
                                                        [@ <?= number_format($lotIn->unit_price, 2) ?>.-]
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        <?php endif; ?>
                                    </td>
                                    <td class="text-end fw-bold text-primary fs-6">
                                        <span class="row-total">0.00</span>
                                    </td>
                                    <td class="text-center">
                                        <button type="button" class="btn btn-outline-danger btn-sm btn-cancel-item" title="ไม่จ่ายรายการนี้">
                                            <i class="bi bi-trash"></i>
                                        </button>
                                        <button type="button" class="btn btn-link btn-sm btn-restore-item d-none">เรียกคืน</button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                        <tfoot>
                            <tr class="table-light">
                                <td colspan="5" class="text-end fw-bold">มูลค่ารวมที่จ่ายจริง:</td>
                                <td class="text-end fw-bold text-danger fs-4" id="grand-total">0.00</td>
                                <td class="text-muted small">บาท</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            </form>

            <div class="text-end mt-4 pt-3 border-top">
                <?= Html::a('ยกเลิก/กลับ', ['index'], ['class' => 'btn btn-light border px-4 me-2']) ?>
                <button type="button" class="btn btn-success btn-lg px-5 shadow" id="btnSubmitIssue">
                    <i class="bi bi-check-all"></i> ยืนยันการจ่ายพัสดุ
                </button>
            </div>
        </div>
    </div>
</div>

<?php
$js = <<< JS
$(document).ready(function() {

    function calculateTotal() {
        let grandTotal = 0;
        $('.item-row').each(function() {
            let row = $(this);
            let qtyInput = row.find('.qty-issued');
            
            if (!qtyInput.prop('disabled')) {
                let qty = parseFloat(qtyInput.val()) || 0;
                let option = row.find('.lot-selector option:selected');
                let price = parseFloat(option.data('price')) || 0;
                let stock = parseFloat(option.data('stock')) || 0;

                // Validation เบื้องต้น
                if (qty > stock) {
                    qtyInput.addClass('is-invalid');
                } else {
                    qtyInput.removeClass('is-invalid');
                }

                let total = qty * price;
                row.find('.row-total').text(total.toLocaleString(undefined, {minimumFractionDigits: 2}));
                grandTotal += total;
            } else {
                row.find('.row-total').text('0.00');
            }
        });
        $('#grand-total').text(grandTotal.toLocaleString(undefined, {minimumFractionDigits: 2}));
    }

    calculateTotal();

    $(document).on('input', '.qty-issued', calculateTotal);
    $(document).on('change', '.lot-selector', calculateTotal);

    $(document).on('click', '.btn-cancel-item', function() {
        let row = $(this).closest('tr');
        row.addClass('table-danger opacity-50');
        row.find('td').css('text-decoration', 'line-through');
        row.find('.qty-issued').val(0).prop('disabled', true);
        row.find('.lot-selector').prop('disabled', true);
        $(this).addClass('d-none');
        row.find('.btn-restore-item').removeClass('d-none');
        calculateTotal();
    });

    $(document).on('click', '.btn-restore-item', function() {
        let row = $(this).closest('tr');
        row.removeClass('table-danger opacity-50');
        row.find('td').css('text-decoration', 'none');
        row.find('.qty-issued').prop('disabled', false).val(row.find('td:eq(2)').text());
        row.find('.lot-selector').prop('disabled', false);
        $(this).addClass('d-none');
        row.find('.btn-cancel-item').removeClass('d-none');
        calculateTotal();
    });

    $('#btnSubmitIssue').click(function() {
        if ($('.is-invalid').length > 0) {
            Swal.fire('คำเตือน', 'กรุณาตรวจสอบจำนวนที่จ่าย (ห้ามเกินยอดคงเหลือในคลัง)', 'warning');
            return;
        }

        Swal.fire({
            title: 'ยืนยันการบันทึก?',
            text: "ระบบจะทำการตัดสต็อกตามล็อตที่ท่านเลือกและบันทึกข้อมูลทันที",
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#198754',
            confirmButtonText: 'ยืนยันจ่ายของ',
            cancelButtonText: 'ตรวจสอบอีกครั้ง'
        }).then((result) => {
            if (result.isConfirmed) {
                Swal.showLoading();
                $.ajax({
                    url: window.location.href,
                    type: 'POST',
                    data: $('#issue-process-form').serialize(),
                    success: function(res) {
                        if (res.success) {
                            Swal.fire('สำเร็จ!', 'จ่ายพัสดุเรียบร้อยแล้ว', 'success')
                            .then(() => { window.location.href = 'index'; });
                        } else {
                            Swal.fire('ผิดพลาด', res.message, 'error');
                        }
                    },
                    error: function() {
                        Swal.fire('Error', 'ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ได้', 'error');
                    }
                });
            }
        });
    });
});
JS;
$this->registerJS($js, View::POS_READY);
?>