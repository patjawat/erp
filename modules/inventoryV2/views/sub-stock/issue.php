...
(function(){
    ...
    $('#btnSaveFinal').on('click', function() {
        ...
        $.post(saveUrl, { warehouse_id: wh, job_type: jobType, reference: reference, items: items })
            .done(function(res) {
                if (res.success) {
                    if (typeof Swal !== 'undefined') Swal.fire('สำเร็จ', (res.order_no ? 'เลขที่ ' + res.order_no + ' — ' : '') + (res.message || 'บันทึกการตัดจ่ายเรียบร้อย'), 'success').then(function() { location.href = window.location.pathname; });
