    ...
    public function actionUpdate($id)
{
    ...
    if (!$model) throw new \yii\web\NotFoundHttpException("ไม่พบข้อมูล");

    // ดึงรายการเก่าเก็บไว้ในตัวแปร array ก่อนที่จะโดน deleteAll
    ...
    if ($this->request->isPost) {
        ...
        try {
            // --- 1. Reverse Stock (หักยอดเก่าออก) ---
            foreach ($oldItems as $oldItem) {
                InventoryService::moveStock(
                    $oldItem->item_code,
                    $model->main_warehouse_id,
                    $oldItem->qty,
                    'OUT', // *** ต้องเป็น OUT เพื่อหักของที่เคยรับเข้าไปออก ***
                    $model->id
                );
            }

            // --- 2. ลบรายการ Detail เก่าทิ้ง ---
            StockDetail::deleteAll(['stock_order_id' => $model->id]);

            // --- 3. บันทึกหัวเอกสารและรายการใหม่ ---
            if ($model->load($this->request->post()) && $model->save()) {
                $details = $this->request->post('StockDetail', []);
