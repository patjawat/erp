...
/**
 * CRUD สำหรับตั้งค่ารายการโรคในครอบครัว (categorise.name = family_disease)
 */
class HealthFamilyDiseaseController extends Controller
{
    ...
    public function actionIndex()
    {
        $models = HealthOption::find()
            ->where(['name' => HealthOption::CATEGORY_FAMILY_DISEASE])
            ->orderBy(['id' => SORT_ASC])
            ->all();
    ...
    public function actionCreate()
    {
        ...
        $model->name   = HealthOption::CATEGORY_FAMILY_DISEASE;
        $model->active = 1;
        ...
        if (Yii::$app->request->isAjax) {
            Yii::$app->response->format = Response::FORMAT_JSON;
            return [
                'title'   => 'เพิ่มรายการโรคในครอบครัว',
                'size'    => 'modal-md',
    ...
    public function actionUpdate($id)
    {
        ...
        if (Yii::$app->request->isAjax) {
            Yii::$app->response->format = Response::FORMAT_JSON;
            return [
                'title'   => 'แก้ไขรายการโรค: ' . Html::encode($model->title),
                'size'    => 'modal-md',
                'content' => $this->renderAjax('_form', ['model' => $model]),
                'footer'  => Html::button('<i class="fas fa-save me-1"></i> บันทึก', ['class' => 'btn btn-primary form-submit', 'data' => ['id' => 'health-option-form']])
                           . Html::button('ปิด', ['class' => 'btn btn-secondary', 'data-bs-dismiss' => 'modal']),
    ...
    public function actionSeed()
    {
        ...
        $inserted = 0;
        foreach (HealthFamilyDisease::defaultList() as $code => $title) {
            $model = new HealthOption();
            $model->name   = HealthOption::CATEGORY_FAMILY_DISEASE;
            $model->code   = $code;
            $model->title  = $title;
    ...
    protected function findModel($id): HealthOption
    {
        $model = HealthOption::findOne([
            'id'   => $id,
            'name' => HealthOption::CATEGORY_FAMILY_DISEASE,
        ]);
        if ($model !== null) {
            return $model;
        }
