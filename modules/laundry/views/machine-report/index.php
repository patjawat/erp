...
<div class="container-fluid py-3">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-between gap-2 mb-3">
        <div><h1 class="h4 fw-bold mb-1"><i class="bi bi-gear-wide-connected me-2"></i><?= Html::encode($this->title) ?></h1><div class="text-body-secondary">สรุปจากรอบที่เริ่มในปีที่เลือก แยกตามเลขครุภัณฑ์ (ใช้น้ำหนักเข้าเครื่อง — หน้างานไม่ได้ชั่งตอนออก)</div></div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden"><div class="table-responsive"><table class="table align-middle mb-0">
        <thead><tr><th class="ps-4">เครื่อง / เลขครุภัณฑ์</th><th class="text-end">เสร็จ</th><th class="text-end">หยุด</th><th class="text-end">กำลังทำ</th><th class="text-end">ผ้าเปื้อนเข้า</th><th class="text-end">ผ้าติดเชื้อเข้า</th><th class="text-end">น้ำหนักรอบหยุด</th><th class="text-end">นาทีรอบเสร็จ</th><th class="text-end pe-4">อัตราบรรทุก</th></tr></thead>
        <tbody>
        <?php foreach ($rows as $row): $s = $row['statistics']; ?><tr>
            ...
            <td class="text-end"><?= number_format($s['soiled_kg'], 3) ?></td><td class="text-end"><?= number_format($s['infectious_kg'], 3) ?></td>
        ...
        <?php if (!$rows): ?><tr><td colspan="9" class="text-center text-muted py-4">ยังไม่มีเครื่องที่ลงทะเบียน</td></tr><?php endif; ?>
        </tbody>
    </table></div></div>
</div>
