 ...
 * Active state from $current_page or $mobileNavActive: home | news | services.
 * "ส่วนตัว" item is a button that toggles #profileDrawer (rendered by
 * layouts/_partials/_profile_drawer.php and included from main layout).
 */
...
<style>
    ...
    .mobile-nav-item { background: transparent; border: 0; cursor: pointer; }
...
<nav class="mobile-bottom-nav fixed-bottom bg-white border-top shadow-lg d-flex align-items-end justify-content-center" role="navigation" aria-label="เมนูหลัก">
    <div class="d-flex align-items-end w-100 justify-content-around px-1 pb-2 pt-2">
        ...
        <button type="button"
                class="mobile-nav-item flex-grow-1 d-flex flex-column align-items-center py-1 text-decoration-none text-secondary"
                data-bs-toggle="offcanvas" data-bs-target="#profileDrawer"
                aria-controls="profileDrawer" aria-label="เปิดเมนูส่วนตัว">
            <i data-lucide="user" class="mb-1"></i>
            <span class="small">ส่วนตัว</span>
        </button>
    </div>
</nav>
