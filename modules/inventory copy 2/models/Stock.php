...
class Stock extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['from_warehouse_id', 'to_warehouse_id', 'qty', 'created_by', 'updated_by'], 'integer'],
            // [['movement_type'], 'required'],
            [['asset_item', 'movement_date', 'expiry_date', 'data_json', 'created_at', 'updated_at', 'qty_check', 'receive_type', 'sum_qty', 'auto_lot', 'movement_type','unit_price', 'total_price', 'q'], 'safe'],
    ...
    //แสดง lot ของรายการวัสดุ
    public function ListItemsByLot()
    {
        return self::find()
            ->where(['name' => 'order_item','movement_type' => 'IN','asset_item' => $this->asset_item])
            ->all();
    }
    ...
    //ผู้ตรวจสอบ อนุมัติ เห็นชอบ
    public function viewChecker()
    {
        try {
            $userId = $this->data_json['checker'];
            $employee = Employees::find()->where(['user_id' => $userId])->one();
            if ($this->data_json['checker']  == 'Y') {
