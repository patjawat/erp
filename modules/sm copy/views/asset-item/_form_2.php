...
<div class="card">
    ...
    <?= $form->field($model, 'category_id')->hiddenInput(['maxlength' => true,'placeholder' => "7440"])->label(false) ?>

    <?= $form->field($model, 'code')->textInput(['maxlength' => true,'placeholder' => "001"])->label("ชนิดครุภัณฑ์") ?>
    </div>
    <div class="col-sm-6 col-md-6">
        ...
        <div class="d-flex justify-content-center">
            <a href="#" class="select-photo">
                <?php if($model->showLogoImg($ref) != false){ ?>
                    <?= Html::img($model->showLogoImg($ref),['class' => 'avatar-profile object-fit-cover rounded shadow','style' =>'max-width: 135px;max-height: 135px;width: 100%;height: 100%;']) ?>
                <?php }else{ ?>
    ...
    <?= $form->field($model, 'name')->hiddenInput(['maxlength' => true,'value'=>'asset_name'])->label(false) ?>
    <?php // $form->field($model, 'id_for_refesh')->hiddenInput(['maxlength' => true,'value'=>])->label(false) ?>
    <?= $form->field($model, 'ref')->hiddenInput(['maxlength' => true,'value'=>$ref])->label(false) ?>

    <?= $form->field($model, 'title')->textInput(['maxlength' => true,'placeholder'=>'เครื่องถ่ายเอกสารระบบใช้หมึกผง ความเร็ว 10 แผ่น/นาที'])->label("ชื่อรายการ") ?>
