            ...
            <div class="table-responsive mt-4">
                <table class="table table-bordered align-middle" id="inboundTable">
                    ...
                    <tbody id="detail-body">
                        ...
                        <tr id="emptyRow" style="<?= !empty($items) && count($items) > 0 && $items[0]->item_code ? 'display:none' : '' ?>">
                            <td colspan="8" class="text-center py-4 text-muted">ยังไม่มีรายการที่ถูกเพิ่ม</td>
                        </tr>
                    </tbody>
...
$(document).off('click', '#btnAddRow').on('click', '#btnAddRow', function(e) {
    ...
    if (!itemId) return Swal.fire('แจ้งเตือน', 'กรุณาเลือกวัสดุ', 'warning');
    ...
    const row = `
        <tr class="item-row">
            ...
            <td>
                <input type="hidden" name="StockDetail[\${currentIndex}][item_code]" value="\${itemId}">
                <strong>\${itemData.item_name}</strong>
            </td>
            <td><input type="text" name="StockDetail[\${currentIndex}][lot_number]" class="form-control form-control-sm"></td>
