    ...
    <div class="card border border-primary border-4 border-top-0 border-end-0 border-start-0">
      <div class="card-body">
        <div class="d-flex justify-content-between">
            ...
            <span class="fs-1"><?=$model->qty?></span>
        </div>
      </div>
...
<table class="table">
  ...
  <tbody>
    <?php foreach($model->listAssets() as $item):?>
    <tr>
      <th scope="row"><?php
      ...
      <td><?=$item->code?></td>
      <td><?=$item->CreateBy()['avatar']?></td>
      <td>-</td>
      <td>-</td>
      <td><?=$item->transaction_type == 'IN' ? $item->qty : ''?></td>
      <td><?=$item->transaction_type == 'OUT' ? -ABS($item->qty) : ''?></td>
      <td><?=$item->qty?></td>
    </tr>
  <?php endforeach;?>
  </tbody>
