    ...
    public function SummaryTotal($status = true)
{
    $query = self::find()
        ->select([
            new \yii\db\Expression('SUM(CASE WHEN e.name = "order_item" THEN COALESCE(i.total_price, 0) ELSE COALESCE(i.total_price, 0) END) AS total')
        ])
        ->alias('e')
        ...
        ->andFilterWhere(['>=', 'e.movement_date', AppHelper::convertToGregorian($this->date_start)])
        ->andFilterWhere(['<=', 'e.movement_date', AppHelper::convertToGregorian($this->date_end)]);
    ...
    // ตัดทศนิยมให้เหลือ 2 ตำแหน่ง โดยไม่ปัดเศษ
    ...
    public function OutSummary($type = null)
    {
        ...
        if ($type) {
            $query->andWhere(['we.warehouse_type' => $type]);
        }
    ...
    // chart สถิติมูลค่าการับเข้าจ่ายออกตามปีงบประมาณ
    ...
    public function stockItemsSummary()
    {
    ...
    //สรุปข้อมูล ยอดยกมา,ยอดใช้ไป,ยอดรับเข้า,มูลค่าคงเหลือ หาจากปีงบประมาน
    public static function priceSummaryOfYear($thaiYear = null)
    {
        $sql = "SELECT 
                we.warehouse_name,
    ...
    // summary คลัง queryc แบบ Dynamic
    public static function buildStockOrderSql($conditions, $params = [], $groupBy = null, $orderBy = null)
    {
...
";
    ...
    public static function buildStockAssetItemSql($conditions, $params = [], $groupBy = null, $orderBy = null)
    {
        ...
        $sql = "SELECT 
            ...
            LEFT JOIN categorise t 
                ON t.code = a.category_id AND t.name = 'asset_type'
            LEFT JOIN stock_events i 
                ON i.asset_item = a.code
            LEFT JOIN stock_events e 
                ON e.id = i.category_id AND e.name = 'order'
    ...
    //             -- 🔹 ยอดคงเหลือสิ้นงวด (ถึง 30 ต.ค. 2025)
    //             SUM(
    //                 CASE 
    //                     WHEN e.movement_date <= :date_end 
    //                         AND i.transaction_type = 'IN'
