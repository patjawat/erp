...
class StockEvent extends Yii\db\ActiveRecord
{
    ...
    public static function buildStockOrderSql($conditions, $params = [], $groupBy = null, $orderBy = null)
    {
        ...
        ----- Auto GROUP / ORDER -----
        $groupFields = [
            'w.warehouse_name', 'w.warehouse_type', 't.title',
            'e.movement_date', 'e.code', 'e.transaction_type',
            'a.category_id', 'i.asset_item', 'a.title'
        ];
        $groupBy = implode(', ', $groupFields);
        $orderBy = 'e.movement_date ASC, a.code DESC';
    ...
    //                                     AND i.order_status = 'success'  
    //                                     AND e.movement_date < :date_start 
    ...
    //                         -- ✅ ยอดเดือนนั้น (:date_start ถึง :date_end)
    //                         SUM(
    //                             CASE 
    //                                 WHEN i.transaction_type = 'IN'  
    //                                     AND i.order_status = 'success' 
    //                                     AND e.movement_date BETWEEN :date_start AND :date_end 
    //                                     AND wi.warehouse_type = 'MAIN' 
    //                                 THEN i.qty 
    //                                 ELSE 0 
    //                             END
