    ...
    public function actionExportExcel()
    {
     ...
     protected function findModel($params)
        ...
        CONCAT(
            ...
            SUM(
                CASE 
                    WHEN (transaction_type = 'OUT' AND from_warehouse_type = 'BRANCH' AND order_status = 'success'  AND MONTH(created_at) = :receive_month AND thai_year = :thai_year) 
                    THEN (qty * unit_price) 
                    ELSE 0 
                END
            ) AS sum_branch,
            ...
            SUM(
                CASE 
                    WHEN (transaction_type = 'OUT' AND from_warehouse_type = 'SUB' AND order_status = 'success' AND MONTH(created_at) = :receive_month AND thai_year = :thai_year) 
                    THEN (qty * unit_price) 
                    ELSE 0 
                END
            ) AS sum_sub
