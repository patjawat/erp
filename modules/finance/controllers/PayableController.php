...
class PayableController extends Controller
{
    ...
    public function actionReview($id)
    {
        ...
        try {
            (new FinancePayableApprovalService())->decide($model, $decision, $note);
            Yii::$app->session->setFlash(
                'success',
                $decision === FinancePayableReview::DECISION_APPROVE
                    ? 'อนุมัติรายการเข้าสู่ทะเบียนเจ้าหนี้แล้ว'
                    : 'ส่งรายการกลับให้ผู้จัดทำแก้ไขแล้ว'
            );
        } catch (\DomainException $e) {
            Yii::$app->session->setFlash('warning', $e->getMessage());
        } catch (\Throwable $e) {
