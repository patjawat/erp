...
<div class="row g-3"><div class="col-md-6"><?= $form->field($model, 'funding_source')->textInput(['maxlength' => true, 'placeholder' => 'เช่น เงินบำรุง / งบประมาณ'])->label('แหล่งเงิน') ?></div><div class="col-md-6"><?= $this->render('@app/modules/finance/views/_date_field', ['model' => $model, 'attr' => 'requested_payment_date', 'label' => 'วันที่ขอจ่าย']) ?></div><div class="col-md-6"><?= $form->field($model, 'payment_method')->dropDownList(FinanceVoucher::paymentMethodOptions())->label('วิธีจ่าย') ?></div><div class="col-12"><?= $form->field($model, 'note')->textarea(['rows' => 3])->label('หมายเหตุ') ?></div></div>
