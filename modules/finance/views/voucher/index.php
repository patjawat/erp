...
<section class="card border shadow-sm mb-4" aria-labelledby="ready-heading">
 ...
 <?php foreach ($ready as $row): ?><tr><td class="fw-semibold"><?= Html::encode($row->payable_no) ?></td><td><?= Html::encode($row->vendor_name_snapshot) ?></td><td><?= Html::encode($row->invoice_no) ?></td><td class="text-nowrap"><?= \app\modules\finance\components\ThaiDate::date($row->due_date) ?></td><td class="text-end text-nowrap"><?= number_format($row->net_amount, 2) ?></td><td class="text-end"><?= Html::a('<i class="bi bi-plus-circle me-1"></i>สร้างร่างฎีกา', ['create', 'payableId' => $row->id], ['class' => 'btn btn-sm btn-primary']) ?></td></tr><?php endforeach; ?>
 </tbody></table></div>
</section>
...
<section class="card border shadow-sm" aria-labelledby="draft-heading">
 ...
 <?php foreach ($drafts as $row): ?><tr><td class="fw-semibold"><?= Html::encode($row->voucher_no) ?></td><td><?= Html::encode($row->vendor_name_snapshot) ?></td><td><?= Html::encode($row->funding_source) ?></td><td><?= \app\modules\finance\components\ThaiDate::date($row->requested_payment_date) ?></td><td><?= Html::encode(FinanceVoucher::paymentMethodOptions()[$row->payment_method]) ?></td><td class="text-end"><?= number_format($row->net_amount, 2) ?></td><td class="text-end"><?= Html::a('ดูรายละเอียด', ['view', 'id' => $row->id], ['class' => 'btn btn-sm btn-outline-primary']) ?></td></tr><?php endforeach; ?>
 </tbody></table></div>
</section>
