...
$this->title = 'อบรม/ประชุม/ดูงาน รายการของฉัน';
$this->params['breadcrumbs'][] = ['label' => 'ภาพรวมอบรม/ประชุม/ดูงาน', 'url' => ['/development/default/dashboard']];
$this->params['breadcrumbs'][] = $this->title;

$developmentTypes = \app\models\Categorise::find()
    ->where(['name' => 'development_type', 'active' => 1])
    ->orderBy(['title' => SORT_ASC])
    ->all();
$statusList = ['' => 'ทั้งหมด'] + ArrayHelper::map(
    \app\models\Categorise::find()->where(['name' => 'leave_status', 'active' => 1])->orderBy(['sort' => SORT_ASC, 'title' => SORT_ASC])->all(),
    'code',
    'title'
);
...
<div class="d-flex flex-wrap gap-2 align-items-center">
    <?= $this->render('@app/modules/development/views/menu_user', ['active' => 'mine']) ?>
</div>
<?php $this->endBlock(); ?>

<div class="container-fluid py-3 px-2 px-sm-3 development-list development-list-modern">
    <div class="row g-3">
        <!-- Filter card -->
        <div class="col-12">
            <?php
            $form = ActiveForm::begin([
                'method' => 'get',
                'action' => ['/development/default/mine'],
                'options' => ['class' => 'development-list-filter-form', 'id' => 'development-mine-filter-form'],
                'fieldConfig' => [
                    'options' => ['class' => 'mb-0'],
            ...
            if (isset($employee->id)) {
                echo Html::hiddenInput('DevelopmentSearch[emp_id]', $employee->id);
            }
            ?>
                ...
                <div class="card-body p-3 p-md-4">
                    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2 mb-3 mb-md-4">
                        <h4 class="text-body mb-0 d-flex align-items-center gap-2">
                            <span class="erp-icon-box bg-primary bg-opacity-10 text-primary rounded-3"><i class="bi bi-list-ul"></i></span>
                            <?= Html::encode($this->title) ?>
                        </h4>
                        <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-pill px-2 py-2">ปีงบประมาณ <?= (int) $thaiYear ?></span>
                    </div>
                    ...
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-12 col-md-3 d-flex gap-2 flex-wrap">
                            ...
                            <?= Html::a('ล้างตัวกรอง', ['/development/default/mine', 'thai_year' => $thaiYear], ['class' => 'btn btn-outline-secondary rounded-pill']) ?>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="d-flex flex-column flex-sm-row flex-wrap align-items-start align-items-sm-center justify-content-between gap-2">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    if (isset($employee->id)) {
                        $exportParams['DevelopmentSearch']['emp_id'] = $employee->id;
                    }
                    $exportUrl = Url::to(array_merge(['/development/default/export-excel'], $exportParams));
                    ?>
                    <?= Html::a('<i class="bi bi-file-earmark-excel me-1"></i>ส่งออก Excel', $exportUrl, ['class' => 'btn btn-success btn-sm rounded-pill']) ?>
                    <div class="dropdown">
                        <button class="btn btn-light border rounded-pill dropdown-toggle btn-sm" type="button" id="dropdownYearMine" data-bs-toggle="dropdown" aria-expanded="false">
                            ปีงบประมาณ <?= (int) $thaiYear ?>
                        </button>
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownYearMine">
                            <?php foreach ($listThaiYear as $y): ?>
                            <li><?= Html::a('ปี ' . (int) $y, ['/development/default/mine', 'thai_year' => $y], ['class' => 'dropdown-item']) ?></li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
                </div>
        ...
        <!-- Activity cards: each card in its own col-12 inside one row -->
        <div class="col-12">
            <div class="row g-3">
                ...
                ?>
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-4 h-100 development-activity-card">
                        <div class="card-body p-3 p-md-4">
                            <div class="d-flex flex-column flex-md-row gap-3 gap-md-4 align-items-start">
                                ...
                                <div class="flex-grow-1 min-w-0 w-100">
                                    ...
                                    <div class="mt-3 d-flex flex-wrap gap-2">
                                        ...
                                        <?= Html::a('<i class="bi bi-pencil me-1"></i> แก้ไข', ['/development/default/update', 'id' => $item->id], [
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-4">
                        <div class="card-body py-5 text-center text-muted">
                            ...
                            <p class="mb-0">ยังไม่มีรายการอบรม/ประชุม/ดูงานของคุณในปีนี้</p>
                            <p class="small mb-0">ลองเปลี่ยนปีงบประมาณหรือล้างตัวกรอง หรือกดปุ่ม <strong>สร้างใหม่</strong> เพื่อขอไปราชการ</p>
                        </div>
                    </div>
                </div>
                <?php endif; ?>
