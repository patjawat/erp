...
/** @var RepairFormSetting $record */
/** @var array $items */
/** @var array $fieldLabels */
/** @var string $templateUrl */

$this->title = 'กำหนดตำแหน่งข้อมูลบน PDF — ใบส่งซ่อม';
$this->params['breadcrumbs'][] = ['label' => 'ระบบงานซ่อม', 'url' => ['/helpdesk/service/index']];
$this->params['breadcrumbs'][] = ['label' => 'แบบฟอร์มใบส่งซ่อม', 'url' => ['/helpdesk/setting/index']];
$this->params['breadcrumbs'][] = 'กำหนดตำแหน่ง';

$this->registerCssFile(Url::to('@web/css/thsarabunnew.css'), ['depends' => [\yii\web\YiiAsset::class]]);
$this->registerCss('.repair-field-chip { font-family: "THSarabunNew", sans-serif; background: transparent !important; border: none !important; box-shadow: none !important; padding: 0 !important; color: #0d6efd !important; }');
...
<div class="alert alert-info border-0 rounded-3 mb-3 d-flex align-items-start gap-2" role="status">
    ...
    <div class="small">
        <strong>การกำหนดตำแหน่ง</strong> — ลากชิ้นงาน (ฟิลด์ข้อมูล) ไปวางบนพื้นที่ PDF ให้ตรงกับช่องในแบบฟอร์มใบส่งซ่อม
        หลังบันทึกแล้วสามารถกดพิมพ์ได้ที่หน้ารายละเอียดงานซ่อม
    </div>
</div>
    ...
    <div class="card-body p-4">
        ...
        <div class="row g-4">
            ...
            <div class="col-12 col-lg-4">
                <div class="mb-3">
                    ...
                    <p class="small text-muted mb-2">
                        เพิ่มได้หลายจุดต่อฟิลด์ เลือกประเภทข้อมูล ปรับขนาด/ความหนา แล้วลากชิปไปวางบนเทมเพลต
                    </p>
                </div>
                <form id="positions-form">
                    <div id="positions-rows" class="d-flex flex-column gap-2 mb-3">
                        ...
                        ?>
                        <div class="position-row d-flex align-items-center gap-2 p-2 rounded-3 border border-1 border-secondary border-opacity-25 flex-wrap <?= !$enabled ? 'opacity-75' : '' ?>"
                             data-item-id="<?= Html::encode($itemId) ?>">
                            <div class="d-flex align-items-center gap-1 flex-grow-1" style="min-width: 0;">
                                <select name="positions[<?= Html::encode($itemId) ?>][key]"
                                        class="form-select position-key-select"
                                        style="width: auto; min-width: 10rem;"
                                        data-item-id="<?= Html::encode($itemId) ?>">
                                    <?php foreach ($fieldLabels as $fk => $fl): ?>
                                    <option value="<?= Html::encode($fk) ?>" <?= $fk === $key ? 'selected' : '' ?>>
                                        <?= Html::encode($fl['label'] ?? $fk) ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                                ...
                                <div class="form-check mb-0 flex-shrink-0">
                                    <input type="checkbox"
                                           name="positions[<?= Html::encode($itemId) ?>][enabled]"
                                           value="1"
                                           class="form-check-input field-enabled-cb"
                                           data-item-id="<?= Html::encode($itemId) ?>"
                                           <?= $enabled ? 'checked' : '' ?>>
                            ...
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <input type="number" min="6" max="24"
                                       name="positions[<?= Html::encode($itemId) ?>][fontSize]"
                                       value="<?= (int) $fontSize ?>"
                                       class="form-control position-font-size"
                                       style="width: 3.5rem;"
                                       data-item-id="<?= Html::encode($itemId) ?>">
                            </div>
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <div class="form-check mb-0">
                                    <input type="checkbox"
                                           name="positions[<?= Html::encode($itemId) ?>][bold]"
                                           value="1"
                                           class="form-check-input position-bold"
                                           data-item-id="<?= Html::encode($itemId) ?>"
                                           <?= $bold ? 'checked' : '' ?>>
                                    <label class="form-check-label small text-muted mb-0">หนา</label>
                                </div>
                            </div>
                            <input type="hidden" name="positions[<?= Html::encode($itemId) ?>][x]"
                                   value="<?= Html::encode($x) ?>"
                                   data-pos-x data-item-id="<?= Html::encode($itemId) ?>">
                            <input type="hidden" name="positions[<?= Html::encode($itemId) ?>][y]"
                                   value="<?= Html::encode($y) ?>"
                                   data-pos-y data-item-id="<?= Html::encode($itemId) ?>">
                            <button type="button"
                                    class="btn btn-outline-danger btn-sm position-remove"
                                    data-item-id="<?= Html::encode($itemId) ?>">
                    ...
                    <div class="mb-3">
                        <button type="button" class="btn btn-outline-primary rounded-3" id="btn-add-position">
                            <i class="bi bi-plus-lg me-1"></i> เพิ่มตำแหน่ง
                        </button>
                    </div>

                    <template id="position-row-tpl">
                        <div class="position-row d-flex align-items-center gap-2 p-2 rounded-3 border border-1 border-secondary border-opacity-25 flex-wrap"
                             data-item-id="__ITEM_ID__">
                            <div class="d-flex align-items-center gap-1 flex-grow-1" style="min-width: 0;">
                                <select name="positions[__ITEM_ID__][key]"
                                        class="form-select position-key-select"
                                        style="width: auto; min-width: 10rem;"
                                        data-item-id="__ITEM_ID__"></select>
                                <input type="hidden" name="positions[__ITEM_ID__][enabled]" value="0">
                                <div class="form-check mb-0 flex-shrink-0">
                                    <input type="checkbox"
                                           name="positions[__ITEM_ID__][enabled]"
                                           value="1"
                                           class="form-check-input field-enabled-cb"
                                           data-item-id="__ITEM_ID__" checked>
                                    <label class="form-check-label small text-muted mb-0">แสดง</label>
                                </div>
                            </div>
                            <div class="d-flex align-items-center gap-1">
                                <label class="small text-muted mb-0 me-1">ขนาด</label>
                                <input type="number" min="6" max="24"
                                       name="positions[__ITEM_ID__][fontSize]"
                                       value="15"
                                       class="form-control position-font-size"
                                       style="width: 3.5rem;"
                                       data-item-id="__ITEM_ID__">
                            ...
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <div class="form-check mb-0">
                                    <input type="checkbox"
                                           name="positions[__ITEM_ID__][bold]"
                                           value="1"
                                           class="form-check-input position-bold"
                                           data-item-id="__ITEM_ID__">
                                    <label class="form-check-label small text-muted mb-0">หนา</label>
                            ...
                            <input type="hidden" name="positions[__ITEM_ID__][x]" value="0"
                                   data-pos-x data-item-id="__ITEM_ID__">
                            <input type="hidden" name="positions[__ITEM_ID__][y]" value="0"
                                   data-pos-y data-item-id="__ITEM_ID__">
                            <button type="button"
                                    class="btn btn-outline-danger btn-sm position-remove"
                                    data-item-id="__ITEM_ID__">
                                <i class="bi bi-trash"></i>
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <?= Html::a('ย้อนกลับ', ['/helpdesk/setting/index'], ['class' => 'btn btn-outline-secondary rounded-3']) ?>
                    </div>
                </form>
            </div>
            ...
            <div class="col-12 col-lg-8">
                <p class="small text-muted mb-2">
                    พื้นที่เทมเพลต (A4) — พื้นหลังเป็น PDF ที่อัปโหลด ลากชิ้นงานไปวางให้ตรงตำแหน่ง
                </p>
                <div class="border rounded-3 overflow-auto bg-secondary bg-opacity-10 d-flex justify-content-center p-3"
                     style="max-height: min(920px, 85vh);">
                    <div id="pdf-canvas-wrapper"
                         class="position-relative shadow-sm"
                         style="width: <?= (int) $canvasW ?>px; height: <?= (int) $canvasH ?>px;">
                        <iframe src="<?= Html::encode($templateUrl) ?>#toolbar=0"
                                ...
                                title="เทมเพลต PDF (พื้นหลัง)"></iframe>
                        <div id="pdf-canvas"
                             class="position-absolute top-0 start-0 w-100 h-100"
                             style="z-index: 1; background: transparent;"
                             data-font-display-scale="<?= Html::encode($fontDisplayScale) ?>">
                            <?php foreach ($items as $item): ?>
                            <?php
                                $itemId        = $item['id'];
                                $key           = $item['key'];
                            ...
                            ?>
                            <div class="position-absolute repair-field-chip text-primary user-select-none <?= !$enabled ? 'd-none' : '' ?>"
                                 data-item-id="<?= Html::encode($itemId) ?>"
                                 ...
                                 draggable="false">
                                <span class="chip-label text-truncate d-inline-block"
                                      style="max-width: 140px;"><?= Html::encode($label) ?></span>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
...
<?php
$saveUrl         = Url::to(['/helpdesk/setting/save-positions']);
...
(function() {
    ...
    function updateChipFont(itemId, fontSize, bold) {
        var chip = canvas.querySelector('.repair-field-chip[data-item-id="' + itemId + '"]');
        if (!chip) return;
    ...
    canvas.querySelectorAll('.repair-field-chip').forEach(attachChipDrag);
    ...
    addBtn.addEventListener('click', function() {
        ...
        chip.className = 'position-absolute repair-field-chip text-primary user-select-none';
    ...
    rowsContainer.querySelectorAll('.position-remove').forEach(function(removeBtn) {
        removeBtn.addEventListener('click', function() {
            var itemId = removeBtn.getAttribute('data-item-id');
            var row    = removeBtn.closest('.position-row');
            var chip   = canvas.querySelector('.repair-field-chip[data-item-id="' + itemId + '"]');
            if (row)  row.remove();
            if (chip) chip.remove();
        });
