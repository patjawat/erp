 ...
 * @since 2.0
 */
class ProductTypeImportController extends Controller
{
    /**
    ...
    public function actionIndex()
    {
        ...
        $sql = 'SELECT t.SUP_TYPE_NAME,TYPE_ID,PROPOTIES_NAME,PROPOTIES_CODE,NUM FROM `supplies_prop` p INNER JOIN supplies_type t ON t.SUP_TYPE_ID = p.TYPE_ID AND t.SUP_TYPE_MASTER_ID = 1 limit 3000;';
        $querys = Yii::$app->db2->createCommand($sql)->queryAll();

        foreach ($querys as $model) {
            $product = Product::findOne(['name' => 'product_type', 'title' => $model['SUP_TYPE_NAME']]);
            try {
                echo $product->title . "\n";
                $newProduct = new Product();
                $newProduct->name = 'product_item';
                $newProduct->category_id = $product->code;
                $newProduct->code = $model['PROPOTIES_CODE'];
                $newProduct->title = $model['PROPOTIES_NAME'];
    ...
    public function UpdatePosition($emp_id)
    {
        try {
            ...
            $model = EmployeeDetail::find()
                ->where(['name' => 'position', 'emp_id' => $emp_id])
