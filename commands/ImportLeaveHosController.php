...
class ImportLeaveHosController extends Controller
{
        ...
        foreach ($querys as $key => $item) {
           ...
           $userCheckId = $this->Person($item['USER_CONFIRM_CHECK_ID']);
            ...
            $leave->data_json = [
                ...
                'approve_3_fullname' => $item['USER_CONFIRM_CHECK'],
    ...
    {
        $querys = Yii::$app->db2->createCommand("SELECT l.*,pt.HR_PERSON_TYPE_NAME FROM `gleave_over` l LEFT JOIN hr_person_type pt ON pt.HR_PERSON_TYPE_ID = l.HR_PERSON_TYPE_ID;")->queryAll();
        foreach ($querys as $key => $item) {
